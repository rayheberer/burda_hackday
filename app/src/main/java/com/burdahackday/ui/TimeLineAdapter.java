...
import com.burdahackday.utils.VectorDrawableUtils;
...
public class TimeLineAdapter extends RecyclerView.Adapter<TimeLineViewHolder> {
    ...
    public void onBindViewHolder(TimeLineViewHolder holder, int position) {
        ...
        if(position==0){
            holder.mTimelineView.setMarker(VectorDrawableUtils.getDrawable(mContext, R.drawable.ic_marker_active, android.R.color.darker_gray));

        }
        else{
            holder.mTimelineView.setMarker(VectorDrawableUtils.getDrawable(mContext, R.drawable.ic_marker, android.R.color.darker_gray));
        }

        if(position==mFeedList.size()-1){
            Picasso.with(mContext).load(timeLineModel.getMessage()).into(holder.mImage);
        }
        else {
            Picasso.with(mContext).load(new File(timeLineModel.getMessage()))
